...
要判断是否可能有键，使用一致的计算方法即可。

### Task2

本任务解决如何将Bloom Filter应用到SST中的问题。Bloom Filter是可以被序列化的，计算好的Bloom Filter会嵌入到SST中，供读取后直接使用。计算key只需要持久化存储filter位数组和k，并不多。

后续只需要`get`方法中，在查找每个SST之前先在对应的Bloom Filter上查找即可。

### Task3

Task3的优化和Task1、Task2没什么关系，它旨在减少SST存储key的内存。由于SST的key是有序的，而且实际使用中，我们很可能会使用诸如`user****`作为键，它们很可能会有较长的相同前缀。因此我们在添加键时可以先计算它与Block的第一个的相同前缀，更换新的编码方式（具体见原文）。
